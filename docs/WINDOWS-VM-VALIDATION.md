...
3. 鼠标移动到每块显示器右侧边缘后，面板宽度达到展开阈值。
4. 鼠标移回显示器中央后，面板隐藏或仅保留窄触发区域。
5. Electron Chromium 调试端点可访问。
6. 单屏和双屏模式分别记录显示器数量、边界和面板位置。

## 内容检查

- Chromium 网页加载
- 笔记创建、自动保存和恢复
- DOCX、PPTX、XLSX 自包含预览
...
因此，代码层的多屏逻辑与自动化测试保留为已通过项；真正的 Windows
双显示器实机验收仍需使用带两个被 Windows 正常识别输出的环境。
